...
select tst.expect('технарь заводит запись своего нового стола', tst.try('T1', $q$insert into rows_page_acl (workspace_id,page_id,responsible_uid,created_by,allowed_uids) values ('W','P9','T1','T1','{T1}')$q$), 'ok:1');
select tst.expect('технарь НЕ заводит стол на чужое имя', tst.try('T1', $q$insert into rows_page_acl (workspace_id,page_id,responsible_uid,created_by) values ('W','P9','T2','T2')$q$), 'error');
...
select tst.expect('ОС НЕ заводит обычный стол', tst.try('OS2', $q$insert into rows_page_acl (workspace_id,page_id,responsible_uid,created_by) values ('W','P10','OS2','OS2')$q$), 'error');
