 ...
 */
export type SbCollectionKey = "deskLoads" | "presence" | "notifications" | "osOrders" | "chat" | "history" | "ratings" | "osDispatchLog" | "orders" | "orderRequests" | "schedule" | "announcements" | "grok" | "personal";
...
export interface Workspace {
  ...
  /**
   * «Технари заполняют сами» (вкладка Owner «Правка столов», 24.09.2026): все
   * технари правят свои столы целиком, включая строки-заказы ОС. Взаимно
   * исключается с `osManagedDesks`; пишет только Owner (`setDeskMode`).
   */
  techFillsAll?: boolean;
  /**
