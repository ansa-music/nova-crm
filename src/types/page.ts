...
export interface PageRow {
  ...
  highlight?: boolean;
  /**
   * СТРОКА-ЗАКАЗ: её ведёт ОС (см. «Стол ОС — источник» в CLAUDE.md).
   *
   * `osUid` — замок: он не пустой, значит статус, цену, клиента и ОС правит
   * только этот ОС (или руководство), а технарю оставлены свои поля —
   * `cells.techLink`, `cells.techNote`, файлы, высота, порядок и снятие
   * подсветки. Держат это не кнопки, а политики Supabase и триггер
   * `desk_rows_guard`: технарь — ответственный за свой стол, и запрет,
   * живущий только в интерфейсе, он обошёл бы сам.
   */
  osUid?: string | null;
  /** Кому адресована строка — сверяется с ответственным за стол при вставке. */
  techUid?: string | null;
  /** Ключ столбца статуса ЭТОЙ вкладки: у вкладок ключи разные. */
  statusKey?: string | null;
  /** Адрес строки-источника в столе ОС. */
  srcPageId?: string | null;
  srcTabId?: string | null;
  srcRowId?: string | null;
  /** Адрес строки-копии в столе технаря (хранится на строке стола ОС). */
  mirrorPageId?: string | null;
  mirrorTabId?: string | null;
  mirrorRowId?: string | null;
  /** Хеш зеркалируемых полей — сверка «доехало ли» без чтения ячеек. */
