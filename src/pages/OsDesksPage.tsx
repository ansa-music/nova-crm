...
/**
 * «Столы ОС» — мониторинг личных таблиц ОС (`page.osDesk`) в одном месте.
 * Owner и любой Тимлид смотрят их напрямую (правило `isOsDeskPage` в
 * firestore.rules), остальные — технари, другие ОС — по разрешению ОС:
 * кнопка «Запросить просмотр» шлёт обычный запрос на просмотр стола, ОС
 * принимает его в колокольчике. Сводка за месяц — только по столам, которые
 * смотрящий и так вправе читать.
 */
        ...
        description={
          permissions.hasFullDeskAccess
            ...
            : permissions.seesOsDesks
              ? `Личные таблицы ОС: что записано сегодня и за ${monthName}, цена и апсейл. Чужие столы вы смотрите напрямую, только на просмотр.`
