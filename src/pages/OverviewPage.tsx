...
import { timeAgo, ymdPartsInTimeZone } from "@/utils/date";
...
/**
 * «Общий дашборд» — the month at a glance for everyone: ratings by «Готово»
 * and by ОС stars, KPIs, orders by day, load, statuses, ОС and months. Built
 * only from aggregates every member may read, so it's the same screen for
 * the Owner, a Тимлид, an ОС, a Технар and a Viewer.
 */
export default function OverviewPage() {
  ...
  if (!permissions.isResolved || (loads === null && !loadsFailed)) {
    return (
      <div className="mx-auto flex max-w-6xl flex-col gap-4 p-5 sm:p-8">
    ...
    <div className="mx-auto flex max-w-6xl flex-col gap-4 p-5 sm:p-8">
      <header className="flex flex-wrap items-end justify-between gap-x-4 gap-y-1">
        <div className="min-w-0">
          <p className="eyebrow mb-1 text-primary">Общий дашборд</p>
          <h1 className="font-serif text-[1.85rem] font-medium tracking-[-0.03em] sm:text-[2.2rem]">{monthName}</h1>
          <p className="mt-1 text-sm text-muted-foreground">
            Все технари за месяц — рейтинги, деньги и заказы. Видят все участники.
          </p>
        </div>
        {totals.updatedAt > 0 && (
