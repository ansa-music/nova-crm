...
function Tile({ item, pathname, onNavigate }: { item: NavItem; pathname: string; onNavigate: () => void }) {
  ...
  // Без предзагрузки по касанию: лист тянут пальцем, начав с плитки, и
  // pointerdown здесь — не намерение перейти. А ошибка import() после деплоя
  // перезагрузила бы вкладку посреди жеста (vite:preloadError). Лист — сам
  // диалог, так что «догадки» preloadRoute тут всё равно молчали бы; страницы
  // меню и так докачивает StartupPreloader, остальное загрузит клик.
  return (
    <NavLink
      to={item.to}
      onClick={onNavigate}
 ...
 *
 * Снаружи — только `Sheet`: всё с хуками (модель, меню аккаунта) живёт в
 * `MoreSheetBody` ВНУТРИ `SheetContent`, а его Radix монтирует лишь пока лист
 * открыт (и на время анимации закрытия). Раньше закрытый лист висел в каркасе
 * у каждого телефона и перерисовывался на каждое сообщение в чате.
 */
