...
import { pathMatches, type NavItem } from "@/config/nav";
 ...
 * (`useAccountMenu`), чтобы два меню не разъезжались.
 */
export function MoreSheet({ open, onOpenChange }: { open: boolean; onOpenChange: (open: boolean) => void }) {
  ...
  // Стол-кнопка нижней панели — «Стол ОС» у ОС и свой стол у технаря; в
  // секции «Столы» они лишние, а «Столы»-список и чужие столы остаются.
  const hiddenKeys = new Set(BOTTOM_BAR_KEYS);
  if (nav.isOs) hiddenKeys.add("os-desk");
