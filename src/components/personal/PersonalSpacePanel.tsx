...
import { ClipboardList, User, Wallet, X } from "lucide-react";
...
export function PersonalSpacePanel({ workspaceId, pageId, uid, onClose }: PersonalSpacePanelProps) {
  const [tab, setTab] = useState<"reports" | "finance" | "notes">("reports");
  ...
  return (
    <div className="flex h-full flex-col">
      <div className="flex items-center gap-2 border-b border-border px-6 py-3">
        ...
        <Tabs value={tab} onValueChange={(v) => setTab(v as typeof tab)} className="ml-4">
          <TabsList>
