 ...
 * прав нет и коллекции туда всё равно не пойдут). Скрыть можно на сутки.
 */
export function SupabaseSqlBanner() {
  ...
  const needed = active && !hidden && missingCount > 0;
  useEffect(() => {
  ...
  async function check() {
    setChecking(true);
    try {
      const states = await Promise.all(WATCHED.map((key) => probeSbTable(key)));
      if (states.every((state) => state === "present")) {
        toast.success("Готово: счётчики, «в сети», уведомления и заказы ОС переезжают в Supabase");
      } else {
