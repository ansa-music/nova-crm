...
import { Check, Loader2, Lock, ShieldCheck, UserCog, X } from "lucide-react";
...
import { grokAppProviderLabel, type GrokAccessRequest, type GrokAccessStub, type WorkspaceMember } from "@/types";

export { canUseGrokMember } from "@/utils/grokPeople";

/**
 * Кто управляет разделом «Грок лимита» (Хикс, 11 Labs, Другие). Это право
 * страницы, а не роль: отметить можно любого участника — Технаря, Admin.
 * Owner и так может всё, в списке его нет. Тот же пикер, что у «Доступа к
 * аккаунту»: группы, «все / снять», поиск.
 */
export function GrokManagersDialog({
  ...
  const [selected, setSelected] = useState<string[]>(() => pickerInitialSelection(managerUids, candidates));

  return (
    <GrokPickerShell
      icon={<UserCog className="h-4 w-4 shrink-0 text-primary" />}
      title={<>Кто управляет «{sectionTitle}»</>}
      description={
        <>
          Отмеченные закрывают и открывают аккаунты «{sectionTitle}» и решают запросы на доступ. Право только этой страницы, ролью не
          выдаётся. Owner может всё и так.
        </>
      }
      onClose={onClose}
      footer={
        <>
          <Button className="min-h-11 gap-1.5 sm:min-h-9" disabled={saving} onClick={() => onSave(selected)}>
      ...
      <ul className="flex flex-col gap-1.5">
        {stubs.map((stub) => {
          ...
          return (
            <li
              key={stub.id}
              className="flex flex-col gap-2 rounded-xl border border-dashed border-border bg-card/40 px-3 py-2.5 sm:flex-row sm:items-center"
            >
