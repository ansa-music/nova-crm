...
- **«Заказы»** (`/orders`, `OrdersPage.tsx`, `orderService.ts`, коллекция `orders`) — биржа между ОС
  ...
  клиент, номер, ссылка, персы, минуты, пожелания; ник ОС подставляется из `osNickValue`, у
  Тимлида/Owner — выбор из «Ответственный»). Технари откликаются (`claims[uid]`, правило пускает
  только свой ключ), выдающий отдаёт заказ одному из них или «Рандом» (среди откликнувшихся со
  столом, иначе среди всех со столом). Назначенный технарь «Забирает в стол»: сессия технаря
  пишет строку в его текущую месячную вкладку (`ensureMonthTab`) через `buildQuickOrderRow`
  (клиент/номер/ОС/перс/мин/ссылка → столбцы, персы-минуты-пожелания → визитка), заказ → `taken`.
  Подписка на заказы живёт только на странице. Старая «Выдача» (`DISPATCH_ENABLED`) — другая
...
- **Таблица стола** (`DataTable.tsx`): первый клик выделяет ячейку, второй клик по выделенной,
  ...
  **Визитка клиента** — `row.extras` (`persons`, `minutes`, `note`; `utils/rowExtras.ts`): кнопка-карточка в
