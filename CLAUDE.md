...
- **«Заказы»** (`/orders`, `OrdersPage.tsx`, `orderService.ts`, коллекция `orders`) — биржа между ОС
  ...
  **срочность** (`urgency`: «Горит» / «Срочный» / «Нейтральный»; нейтральный бейджем не метится).
  ...
  через `buildQuickOrderRow` (клиент/номер/цена/ОС/перс/мин/ссылка/дедлайн → столбцы,
  персы-минуты-пожелания-ссылка → визитка) и переводит заказ в `taken`. Столбцы берутся у ТОЙ
